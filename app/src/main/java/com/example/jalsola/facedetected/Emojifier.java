...
public class Emojifier {
    ...
    static Bitmap detectFacesandOverlayEmoji(Context context, Bitmap picture) {
        ...
        Log.e(LOG_TAG, "detectFacesandOverlayEmoji: number of faces = " + faces.size());
        ...
        if (faces.size() == 0) {
            ...
            for (int i = 0; i < faces.size(); i++) {
                ...
                resultBitmap = addBitmmapToFace(resultBitmap, emojiBitmap, face);
            }
        }
    ...
    private static Emoji whichEmoji(Face face) {
        ...
        boolean rigthEyeClosed = face.getIsRightEyeOpenProbability() < EYE_OPEN_PROB_THRESHOLD;

        // Determine and log the appropriate emoji
        Emoji emoji;
        if (smiling) {

            if (leftEyeClosed && !rigthEyeClosed) {
                emoji = Emoji.LEFT_WINK;
            } else if (rigthEyeClosed && !leftEyeClosed) {
    ...
    private static Bitmap addBitmmapToFace(Bitmap backgroundBitmap, Bitmap emojiBitmap, Face face) {
